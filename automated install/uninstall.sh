...
removeNoPurge() {
    ...
    if [[ -d "/srv/www/htdocs/pihole" ]]; then
        WWWROOT="/srv/www/htdocs"
    fi
    ${SUDO} rm -rf ${WWWROOT}/admin &> /dev/null
    ${SUDO} rm -rf ${WWWROOT}/pihole &> /dev/null
    ${SUDO} rm -f ${WWWROOT}/index.lighttpd.orig &> /dev/null
